...
module.exports = class ViessmannOAuth2Client extends OAuth2Client {
  ...
  */
  ...
  async setDhwTemp({
    ...
    try {
      if (!PATHS.HOT_WATER_TARGET) {
        throw new Error('HOT_WATER_TARGET path is not defined in config');
      }

      return await this.post({
        path: `/features/installations/${installationId}/gateways/${gatewaySerial}/devices/${deviceId}/features/${PATHS.HOT_WATER_TARGET}/commands/setTargetTemperature`,
        json: { temperature: value },
        timeout: this.constructor.DEFAULT_TIMEOUT,
      });
    } catch (error) {
      this.error('Error in setDhwTemp:', error);
      throw new OAuth2Error(error.message || 'Failed to set hot water temperature');
    }
  }
  ...
  async setDhwOneTimeCharge({
    ...
    try {
      if (!PATHS.HOT_WATER_CHARGE) {
        throw new Error('HOT_WATER_CHARGE path is not defined in config');
      }

      if (process.env.DEBUG) {
        this.log('setDhwOneTimeCharge::value', `/features/installations/${installationId}/gateways/${gatewaySerial}/devices/${deviceId}/features/${PATHS.HOT_WATER_CHARGE}/commands/${value}`);
      }

      return await this.post({
        path: `/features/installations/${installationId}/gateways/${gatewaySerial}/devices/${deviceId}/features/${PATHS.HOT_WATER_CHARGE}/commands/${value}`,
        json: { },
        timeout: this.constructor.DEFAULT_TIMEOUT,
      });
    } catch (error) {
      this.error('Error in setDhwOneTimeCharge:', error);
      throw new OAuth2Error(error.message || 'Failed to set hot water one time charge');
    }
  }
  ...
  async setHeatingTemp({
    ...
    try {
      if (!PATHS.HEATING_TARGET) {
        throw new Error('HEATING_TARGET path is not defined in config');
      }

      return await this.post({
        path: `/features/installations/${installationId}/gateways/${gatewaySerial}/devices/${deviceId}/features/${PATHS.HEATING_TARGET}/commands/setTemperature`,
        json: { targetTemperature: value },
        timeout: this.constructor.DEFAULT_TIMEOUT,
      });
    } catch (error) {
      this.error('Error in setHeatingTemp:', error);
      throw new OAuth2Error(error.message || 'Failed to set heating temperature');
    }
  }
  ...
  async setMainOperatingMode({
    ...
    try {
      if (!PATHS.HEATING_MODE) {
        throw new Error('HEATING_MODE path is not defined in config');
      }

      return await this.post({
        path: `/features/installations/${installationId}/gateways/${gatewaySerial}/devices/${deviceId}/features/${PATHS.HEATING_MODE}/commands/setMode`,
        json: { mode: value },
        timeout: this.constructor.DEFAULT_TIMEOUT,
      });
    } catch (error) {
      this.error('Error in setMainOperatingMode:', error);
      throw new OAuth2Error(error.message || 'Failed to set operating mode');
    }
  }
  ...
  async setDhwTemp2({
    ...
    try {
      if (!PATHS.HOT_WATER_TARGET_2) {
        throw new Error('HOT_WATER_TARGET_2 path is not defined in config');
      }

      return await this.post({
        path: `/features/installations/${installationId}/gateways/${gatewaySerial}/devices/${deviceId}/features/${PATHS.HOT_WATER_TARGET_2}/commands/setTargetTemperature`,
        json: { temperature: value },
        timeout: this.constructor.DEFAULT_TIMEOUT,
      });
    } catch (error) {
      this.error('Error in setDhwTemp2:', error);
      throw new OAuth2Error(error.message || 'Failed to set hot water temperature 2');
    }
  }
