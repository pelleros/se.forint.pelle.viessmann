...
const { CAPABILITIES } = require('../drivers/vicare/config');
...
module.exports = class ViessmannOAuth2Client extends OAuth2Client {
  ...
  async onInit() {
    ...
    this._clientSecret = '5M5nhkBfkWZCGfLZYcTL-l7esjPUN7PpZ4rq8k4cmys';
  }
  ...
  async onGetTokenByCode({ code }) {
    ...
    body.append('code_verifier', ViessmannOAuth2Client.CODE_VERIFIER);
  ...
  */
  async getGateways() {
    ...
    if (process.env.DEBUG) {
      this.log('[ViessmannOAuth2Client::getGateways] result:', JSON.stringify(result, null, 3));
    }
    return result;
  }
  ...
  */
  ...
  async setHeatingTemp({
    ...
    return this.post({
      path: `/features/installations/${installationId}/gateways/${gatewaySerial}/devices/${deviceId}/features/${CAPABILITIES.HEATING_TARGET_TEMP.featureName}/commands/setTemperature`,
  ...
  async setMainOperatingMode({
    ...
    return this.post({
      path: `/features/installations/${installationId}/gateways/${gatewaySerial}/devices/${deviceId}/features/${CAPABILITIES.HEATING_MODE.featureName}/commands/setMode`,
      json: { mode: value },
    });
  }
