...
module.exports = class MyBrandOAuth2Client extends OAuth2Client {
  ...
  //   this.log('onHandleRefreshTokenError::response:', response);
  //   super.onHandleRefreshTokenError({ response });
  // }

  // async onHandleRefreshTokenResponse({ response }) {
  //   this.log('onHandleRefreshTokenResponse::response:', response);
  //   super.onHandleRefreshTokenResponse({ response });
  // }

  // async onRequestResponse({
  //   req, url, opts, response, didRefreshToken,
  // }) {
  //   this.log('onRequestResponse.url', url);
  //   this.log('onRequestResponse.response', response);
  ...
  /*/
    const response = await fetch(this._tokenUrl, {
      ...
      body: new URLSearchParams({
        ...
        code_verifier: Homey.env.CODE_VERIFIER,
    ...
    this.log('code_verifier:', Homey.env.CODE_VERIFIER);
    const body = await response.json();
    this.log('onGetTokenByCode', body);
